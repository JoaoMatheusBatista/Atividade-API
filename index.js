...
const logApiUrl = 'https://www.piway.com.br/unoesc/api/inserir/log/' + MATRICULA;
...
async function logRequest(message) {
    console.log(message);
    const logs = await fetch (`${logApiUrl}/fixer-api/inserir/${message}`);
    await viewLogs();
}
...
async function convertCurrency(amount, fromCurrency, toCurrency) {
    ...
    await logRequest(`${amount} ${fromCurrency} = ${convertedAmount.toFixed(2)} ${toCurrency}`);
...
async function viewLogs() {
    try {
        ...
        logs.forEach(log => {
            ...
            logItem.innerHTML = `
                idLog: ${log.idlog} <br>Horário: ${log.log} <br>API: ${log.api} <br>Método: ${log.metodo} <br>Resultado: ${log.resultado} <br>
...
async function deleteLog(idLog) {
        const del = await fetch (`${deleteLogUrl}/${idLog}/aluno/${MATRICULA}`);
